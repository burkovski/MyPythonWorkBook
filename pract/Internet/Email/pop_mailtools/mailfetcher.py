...
from pop_mailtools.mailtool import MailTool, SilentMailTool  # Суперкласс упр. трассировкой
...
class MailFetcher(MailTool):
    ...
    def decode_full_text(self, msg_bytes):
        ...
        if text is None:
            ...
            text.extend(('', "-- Sorry: pop_mailtools cannot decode this mail content! --"))
        return text

    def download_message(self, msg_num):
        """
        загружает полный текст одного сообщения по указанному относительному
        номеру POP msg_num; анализ содержимого выполняет вызывающая программа
        """
