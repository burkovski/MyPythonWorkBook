...
class IdentifierTable:
    ...
    def make_table(self, *, share_exception=False):
        self.table = {line[self.group_names[0]]: (line[self.group_names[1]], line[self.group_names[2]])
    ...
    def write_table(self, path=None):
        ...
        template = "| {0:<15}| {1:<10}| {2:<10}| {3:<10}|"
        sep_chr = '-'
        sep_str = "+{}+{}+{}+{}+".format(sep_chr*16, sep_chr*11, sep_chr*11, sep_chr*11)
        hdr = template.format("KEY", "NAME", "NUMBER", "VALUE")
        with open(path, 'w') as file:
            ...
            for id_name in self.table:
                id_num, id_val = self.table[id_name]
                print(template.format(hash(id_name), id_name, id_num, id_val))
                print(sep_str)
            sys.stdout = sys.__stdout__


if __name__ == "__main__":
    group_names = ("var_id", "var_num", "var_val")
    raw_str = r"^(?P<{0}>\w+)\s*=\s*(?P<{1}>\d+.?\d+)\s*(?P<{2}>(?:Kilo|Mega|Giga|Tera)?bytes|[KMGT]?b)\s*$"
    template_str = raw_str.format(*group_names)
    id_table = IdentifierTable(r"C:\Users\HP\Documents\ids.txt", template_str, group_names)
