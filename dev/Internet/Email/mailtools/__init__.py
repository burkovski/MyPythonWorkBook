"""
############################################################################
пакет mailtools: интерфейс к почтовому серверу, используется
PyMailCGI; реализует загрузку, отправку, анализ,
...
и многое другое; загружает все почтовые сообщения;
не содержит специальной поддержки
...
необходимо использовать синтаксис импортирования относительно
...
- TBD: в walk_named_parts, возможно, следовало бы перешагивать через нетипичные
